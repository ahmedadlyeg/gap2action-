...
import { ArrowLeft, Edit2, Camera, LayoutTemplate, Zap, PenLine } from 'lucide-react';
import { Button } from '@/components/ui/button';
import {} from '@/components/ui/badge';
import { useAuth } from '@/context/AuthContext';
import { templatesApi, categoriesApi, type ApiTemplate, type ApiCategory } from '@/services/api';
...
export function TemplateLanding() {
  ...
  useEffect(() => {
    ...
    templatesApi.get(id).then(tpl => {
      setTemplate(tpl);
      if (tpl?.categoryId) {
        categoriesApi.get(tpl.categoryId).then(setCategory).catch(() => {});
      }
    }).catch(() => {});
  }, [id]);
  ...
  const saveTagline = async () => {
    if (template) {
      await templatesApi.update(template.id, { tagline: taglineDraft }).catch(() => {});
      setTemplate(t => t ? { ...t, tagline: taglineDraft } : t);
    }
  ...
  const handleCoverUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async ev => {
      ...
      await templatesApi.update(template.id, { coverImageUrl: dataUrl }).catch(() => {});
      setTemplate(t => t ? { ...t, coverImageUrl: dataUrl } : t);
    };
  ...
  if (!template) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center px-4">
        ...
        <p className="text-slate-500 text-sm">The template you are looking for does not exist or has been removed.</p>
        ...
        <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          {isAdmin ? (
            editingTagline ? (
              ...
              <button onClick={() => setEditingTagline(true)} className="text-left group/tl flex items-center gap-2">
      ...
      <div className="max-w-3xl mx-auto px-4">
        <div className="max-w-lg mx-auto mt-8 mb-6">
          <div className="relative group/cover rounded-2xl overflow-hidden shadow-xl aspect-video">
            {template.coverImageUrl ? (
              <img src={template.coverImageUrl} alt={template.name} className="w-full h-full object-cover" />
            ) : (
        ...
        <div className="mt-6 bg-white rounded-2xl shadow-sm p-6 md:p-8">
          ...
          <EditableBlock
            ...
            onSave={val => templatesApi.update(template.id, { definition: val })
              .then(() => setTemplate(t => t ? { ...t, definition: val } : t)).catch(() => {})}
        ...
        <div className="mt-6 bg-white rounded-2xl shadow-sm p-6 md:p-8">
          ...
          <EditableBlock
            ...
            onSave={val => templatesApi.update(template.id, { explanation: val })
              .then(() => setTemplate(t => t ? { ...t, explanation: val } : t)).catch(() => {})}
          ...
          {sections.length > 0 && (
            <div className="mt-6">
              <p className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">Assessment Sections</p>
        ...
        <div className="mt-8 mb-12">
          {template.status === 'Active' ? (
            <div className="flex justify-center">
              <Button size="lg" className="px-10" onClick={() => navigate(`/events/new?templateId=${template.id}`)}>
